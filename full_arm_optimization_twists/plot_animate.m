function plot_animate(x, input, data_file)
    ...
    L7 = Link('d', 0, 'a', a(7)*0, 'alpha', alpha(7)*0,'offset', offset(7)*0);
    
    m = 200;
    input.m       = m;
    q_min         = input.q_min;
    q_max         = input.q_max;
    des_poses     = trajectory_pose_read(data_file{2}, m);
    pd            = des_poses(:,:,:,:);
    ...
    for i = 1:m
        ...
        mini_chain_2.plot(q_R(:,i)', 'noshadow','workspace',ws, 'view',[-165 45],'tile1color',[10 1 1],'delay',0.01,'jointdiam',1.3,'scale',0.7,'wrist','jointcolor',[0.5 0.5 0.5],'linkcolor',[0.7 0.0 0.0])
        hold on

        mini_chain_1.plot(q_L(:,i)', 'noshadow','workspace',ws, 'view',[-165 45],'tile1color',[10 1 1],'delay',0.01,'jointdiam',1.3,'scale',0.7,'wrist','jointcolor',[0.5 0.5 0.5],'linkcolor',[0.7 0.0 0.0])
