function [thetalist, success] ...
         = IKinSpace_modified(Slist, M, TR, T, thetalist0, eomg, ev, init)
...
Tsb = TR * FKinSpace(M, Slist, thetalist);
...
while err && i < maxiterations
    J = Adjoint(TR) * JacobianSpace(Slist, thetalist);
    ...
    thetalist_ = thetalist + 0.1 * JINV * Vs; 
    
    
    Tsb = TR * FKinSpace(M, Slist, thetalist_);
    Vs  = Adjoint(Tsb) * se3ToVec(MatrixLog6(TransInv(Tsb) * T));
     
    % line search
    for j=1:numel(alpha)
        theta_null = 0.05 * alpha(1) * (eye(7) - J'*JINV')  * null_space(Slist, thetalist)';
        
        Tsb_ = TR * FKinSpace(M, Slist, thetalist_+theta_null);
...
function dcdq = null_space(Slist, q_init)
    global input
    q  = q_init;
    %     Jq = J(q);
    %     d  = det(Jq(1:3,:)*Jq(1:3,:)');
    %     q_lim_grad = exp((q - input.q_min')) + exp((q - input.q_max'));
    q_lim_grad = (q - (input.q_min' + input.q_max')/2) ./ (input.q_max' - input.q_min');
